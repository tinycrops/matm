# traj_retrieval/strategies/async_strategies.py
# Async strategy implementations for LLM experiments
...
class AsyncBaseExperimentStrategy(BaseExperimentStrategy):
    """
    Base class for async strategies.
    Adds the async execute() method to the base interface.
    """
    ...
    async def execute(
        ...
        **kwargs,
        ...
        This is the main entry point for each async strategy.
...
class ActionStringAsyncStrategy(AsyncBaseExperimentStrategy, HybridRerankingMixin):
    """
    ...
    async def _retry_llm_call_with_parsing(
        self,
        call_func,
        parse_func,
        context: str,
    ...
    async def _call_llm_normal(
        ...
        action, additional_info = await self._retry_llm_call_with_parsing(
            normal_call,
            normal_parse,
    ...
    async def _call_llm_structured(
        ...
        reasoning, action = await self._retry_llm_call_with_parsing(
            structured_call,
            structured_parse,
    ...
    async def execute(
        ...
        **kwargs,
        ...
        2. only_structured: Structured call → Ultimate fallback
        3. normal_then_structured: Normal → Structured with reranking → Ultimate fallback

        CRITICAL:
        - Ultimate fallback action MUST be in admissible_actions or ValueError is raised
        - Never uses "first action" as fallback - strict requirement
        """
        ...
        if env_handler is not None:
            try:
                # Get task name from the environment handler for task-specific examples
                task_metadata = env_handler.get_task_metadata()
                task_name = task_metadata.get("task_name")
                system_message = env_handler.build_system_message(task_name)
                print(
        ...
        # CRITICAL: Capture system_message and user_message in debug_info for logging
        # This ensures complete prompt reconstruction in debug files
        debug_info["system_message"] = system_message if system_message else ""
        debug_info["user_message"] = user_message

        # Route to appropriate execution path
        if execution_path == "only_normal":
            return await self._execute_only_normal(
                aclient,
                limiter,
                requests_sem,
                model,
            ...
            return await self._execute_only_structured(
                aclient,
                limiter,
                requests_sem,
                model,
            ...
            return await self._execute_normal_then_structured(
                aclient,
                limiter,
                requests_sem,
                model,
    ...
    async def _execute_only_normal(
        self,
        aclient,
        limiter,
        requests_sem,
        ...
        try:
            ...
            action, additional_info = await self._call_llm_normal(
                aclient=aclient,
                limiter=limiter,
            ...
            debug_info["ultimate_fallback_used"] = True
            debug_info["ultimate_fallback_reason"] = f"Normal call failed: {str(error)}"
    ...
    async def _execute_only_structured(
        self,
        aclient,
        limiter,
        requests_sem,
        ...
        try:
            ...
            reasoning, action = await self._call_llm_structured(
                aclient=aclient,
                limiter=limiter,
                requests_sem=requests_sem,
                model=model,
            ...
            debug_info["reasoning"] = reasoning
            debug_info["direct_structured_used"] = True
    ...
    async def _execute_normal_then_structured(
        self,
        aclient,
        limiter,
        requests_sem,
        ...
        try:
            ...
            try:
                ...
                debug_info["hybrid_reranking_fallback_used"] = True
                debug_info["structured_fallback_used"] = True
                debug_info["reasoning"] = reasoning
                debug_info["filtered_actions_count"] = len(filtered_actions)
            ...
            debug_info["ultimate_fallback_used"] = True
            debug_info["ultimate_fallback_reason"] = f"API error: {str(api_error)}"
            debug_info["reasoning"] = reasoning
            print(
                f"[ACTION-STRING:{episode_id}] Ultimate Fallback Action: '{action}'",
                flush=True,
